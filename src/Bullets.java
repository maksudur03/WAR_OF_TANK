...
public class Bullets {
	...
	public Bullets(int x, int y, boolean good, Direction dir, WAROFTANK war) {
		this(x, y, dir);
		this.good = good;
		this.war = war;
	}
	...
	private void move() {
		...
		if (x < 0 || y < 0 || x > WAROFTANK.Fram_width
				|| y > WAROFTANK.Fram_length) {
			live = false;
		}
	}
	...
	public boolean hitTank(Tank t) { 
		...
		if (this.live && this.getRect().intersects(t.getRect()) && t.isLive()
			...
			Brust e = new Brust(t.getX(), t.getY(), war);
			war.Brusts.add(e);
			if (t.isGood()) {
	...
	public boolean hitWall(BreakWall w) { 
		if (this.live && this.getRect().intersects(w.getRect())) {
			this.live = false;
			this.war.otherWall.remove(w); 
			this.war.HomeWall.remove(w);
			return true;
		}
		return false;
	}
	public boolean hitBullet(Bullets w){
		if (this.live && this.getRect().intersects(w.getRect())){
			this.live=false;
			this.war.bullets.remove(w);
                        
			return true;
		}
		return false;
	}
	public boolean hitWall(SteelWall w) { 
		if (this.live && this.getRect().intersects(w.getRect())) {
			this.live = false;
			return true;
		}
	...
	public boolean hitHome() { 
		if (this.live && this.getRect().intersects(war.Home.getRect())) {
			this.live = false;
			this.war.Home.setLive(false); 
			return true;
		}
		return false;
	}
